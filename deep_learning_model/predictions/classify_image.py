...
class ImageClassifier:
    def __init__(self):
        ...
        self.classifier_0 = Classifier(0).load_from_checkpoint(
            models_dir_path + '0' + '.ckpt', team_num=0)
        self.classifier_0.eval()
        self.classifier_0.freeze()

        self.classifier_1 = Classifier(1).load_from_checkpoint(
            models_dir_path + '1' + '.ckpt', team_num=1)
        self.classifier_1.eval()
        self.classifier_1.freeze()

        self.classifier_2 = Classifier(2).load_from_checkpoint(
            models_dir_path + '2' + '.ckpt', team_num=2)
        self.classifier_2.eval()
        self.classifier_2.freeze()

        self.classifier_3 = Classifier(3).load_from_checkpoint(
            models_dir_path + '3' + '.ckpt', team_num=3)
        ...
        if use_individual_models:
            ...
            if team_num == 0:
                player_id = torch.argmax(
                    torch.exp(self.classifier_1(image)), dim=1)
                print(player_id)
                return CLASSES_1[player_id]
            elif team_num == 1:
                player_id = torch.argmax(
                    torch.exp(self.classifier_2(image)), dim=1)
                print(player_id)
