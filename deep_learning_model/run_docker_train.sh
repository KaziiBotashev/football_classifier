...
echo $(docker build -t football_classifier_train .)
echo "Building Docker image is finished."
echo "Running Docker container..."
if [ ! "$(docker ps -q -f name=football_classifier_training)" ]; then
    if [ "$(docker ps -aq -f status=exited -f name=football_classifier_training)" ]; then
        # cleanup
        docker rm football_classifier_training
    fi
    # run your container
    docker run -d --gpus all --name football_classifier_training -p 8888:8888 -it football_classifier_train
fi
docker start football_classifier_training
docker attach football_classifier_training
