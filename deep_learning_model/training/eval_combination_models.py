...
if __name__ == '__main__':
    ...
    model_0 = Classifier(0).load_from_checkpoint(models_dir_path + '0' +'.ckpt', team_num = 0).to(device)
    model_0.eval()
    model_0.freeze()

    model_1 = Classifier(1).load_from_checkpoint(models_dir_path + '1' +'.ckpt', team_num = 1).to(device)
    model_1.eval()
    model_1.freeze()

    model_2 = Classifier(2).load_from_checkpoint(models_dir_path + '2' +'.ckpt', team_num = 2).to(device)
    model_2.eval()
    model_2.freeze()

    model_3 = Classifier(3).load_from_checkpoint(models_dir_path + '3' +'.ckpt', team_num = 3).to(device)
    model_3.eval()
    model_3.freeze()
    ...
    indices_alphabetical = [0, 1, 12, 18, 19, 20, 21, 22, 23, 24, 2, 3, 4, 5, 6, 7, 8, 
...
#            print(x[i].shape)
            if pred_0 == 0:
                player_id = torch.argmax(torch.exp(model_1(x[i].unsqueeze(0))), dim = 1)
#                print(player_id)
                predicted_labels.append(indices_alphabetical[team1[player_id]])
            elif pred_0 == 1:
                player_id = torch.argmax(torch.exp(model_2(x[i].unsqueeze(0))), dim = 1)
#                print(player_id)
